...
fn main() {
    ...
    println!(
        "Tert-butanol: {} atoms, {} bonds",
        tert_butanol.atom_count(),
        tert_butanol.bond_count()
    );
    ...
    println!(
        "Ethanol contains alcohol pattern: {}",
        ethanol.contains_substructure(&alcohol_pattern)
    );
    println!(
        "Benzene contains alcohol pattern: {}",
        benzene.contains_substructure(&alcohol_pattern)
    );
    println!(
        "Tert-butanol contains alcohol pattern: {}",
        tert_butanol.contains_substructure(&alcohol_pattern)
    );

    // Find all aromatic carbons in benzene
    let aromatic_matches = benzene.find_all_substructure_matches(&aromatic_carbon);
    println!(
        "Number of aromatic carbons in benzene: {}",
        aromatic_matches.len()
    );

    // Use SMARTS pattern matching directly
    let hydroxyl_matches = tert_butanol.find_smarts_matches("O").unwrap();
    println!(
        "Number of oxygen atoms in tert-butanol: {}",
        hydroxyl_matches.len()
    );

    // Complex SMARTS pattern - quaternary carbon pattern
    let tertiary_carbon = MolecularGraph::from_smarts("C(C)(C)C").unwrap();
    println!(
        "Tert-butanol contains tertiary carbon: {}",
        tert_butanol.contains_substructure(&tertiary_carbon)
    );

    // Complex SMARTS pattern
    let quaternary_carbon = MolecularGraph::from_smarts("C(C)(C)(C)C").unwrap();
    println!(
        "Tert-butanol contains quaternary carbon: {}",
        tert_butanol.contains_substructure(&quaternary_carbon)
    );
}
